    ...
    <header class="studio-topbar">
      <div class="studio-topbar-main">
        ...
        <div class="studio-identity">
          ...
          <span class="studio-episode-chip">Episode {{ episodeNumber }}</span>
          <div class="studio-meta-row">
            ...
            <span class="studio-meta-inline">{{ chars.length }} character · {{ sbs.length }} shot</span>
          </div>
        </div>
      </div>
      ...
      <div class="studio-topbar-side">
        <div class="studio-actions">
          ...
          <button class="btn btn-primary" @click="panel = mergeUrl ? 'export' : (sbs.length ? 'production' : 'script')">
            ...
            {{ mergeUrl ? 'View Final Video' : (sbs.length ? 'Continue' : 'Start production') }}
          </button>
        </div>
      </div>
    </header>
    ...
    <main class="main">
      ...
      <div v-if="panel === 'script'" class="content-panel">
        ...
        <div v-if="scriptStep === 0" class="step-editor">
          <div class="step-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">Original Content</span>
              </div>
            </div>
            <div class="toolbar-right">
              <span v-if="rawLen" class="char-count">{{ rawLen }} chars</span>
              <button class="btn btn-sm" @click="saveRaw(); toast.success('Saved')">
          ...
          <textarea
            class="fill-textarea"
            v-model="localRaw"
            placeholder="Paste the novel, story outline, or storyboard description..."
          />
        </div>
        ...
        <div v-else-if="scriptStep === 1" class="step-editor">
          <div class="step-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">AI Rewrite</span>
              </div>
            </div>
            <div class="toolbar-right">
              <span v-if="scriptLen" class="char-count">{{ scriptLen }} chars</span>
          ...
          <div v-if="!scriptContent && !rn" class="step-empty">
            ...
            <div class="empty-title">AI Rewrite to Formatted script</div>
            <div class="empty-desc">You can have AI organize the original content into a formatted script, or skip this step and use the original content directly to continue extracting characters and scenes.</div>
          ...
          <div v-else-if="rn && rt === 'script_rewriter'" class="step-loading">
            ...
            <div class="loading-text">Rewriting script...</div>
          </div>
          <textarea v-else class="fill-textarea" v-model="localscript" placeholder="Formatted script content..." />
        </div>
        ...
        <div v-else-if="scriptStep === 2" class="step-editor">
          <div class="step-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">Extract characters & scenes</span>
              </div>
            </div>
            <div class="toolbar-right">
              <span v-if="chars.length" class="char-count">{{ chars.length }} character · {{ scenes.length }} scene</span>
          ...
          <div v-if="!chars.length && !rn" class="step-empty">
            ...
            <div class="empty-title">fromscriptExtract characters & scenes</div>
            <div class="empty-desc">AI auto-analyzes the script, extracts character info and scene list, and intelligently dedups/merges with project data</div>
          ...
          <div v-else-if="rn && rt === 'extractor'" class="step-loading">
            <Loader2 :size="24" class="animate-spin" style="color:var(--accent)" />
            <div class="loading-text">Extracting characters and scenes...</div>
          </div>
          <div v-else class="extract-stage">
            <aside class="card extract-summary">
              <div class="extract-summary-kicker">Extraction Board</div>
              <div class="extract-summary-title">character & scene Results</div>
              <div class="extract-summary-desc">characters and scenes extracted from the script are now in the database. Confirm names, roles, and descriptions are ready for the next production steps.</div>
              ...
              <div class="extract-summary-note">If character descriptions are too brief, we recommend filling in their features before assigning voices and generating portraits.</div>
            </aside>
                ...
                <div v-for="c in chars" :key="c.id" class="extract-row">
                  ...
                  <div class="extract-info">
                    <div class="extract-name-row">
                      ...
                      <span class="tag">{{ c.role || 'character' }}</span>
                    </div>
                    <div class="extract-meta wrap">{{ c.description || c.appearance || c.personality || 'No description' }}</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="extract-list">
                <div v-for="s in scenes" :key="s.id" class="extract-row">
                  ...
                  <div class="extract-info">
                    ...
                    <div class="extract-meta wrap">{{ s.description || s.time || 'scene description pending' }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-else-if="scriptStep === 3" class="step-editor">
          <div class="step-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">Assign voices</span>
              </div>
            </div>
            <div class="toolbar-right">
              <span v-if="charsvoiced" class="char-count">{{ charsvoiced }}/{{ chars.length }} Assigned</span>
              <span v-if="voicesampleCount" class="char-count">{{ voicesampleCount }}/{{ charsvoiced }} voice samples</span>
          ...
          <div v-else-if="rn && rt === 'voice_assigner'" class="step-loading">
            <Loader2 :size="24" class="animate-spin" style="color:var(--accent)" />
            <div class="loading-text">isinAssign voices...</div>
          </div>
          <div v-else class="voice-stage">
            <aside class="card voice-stage-panel">
              <div class="voice-stage-kicker">voice Casting</div>
              <div class="voice-stage-title">character voice Assignment</div>
              <div class="voice-stage-desc">Pick a voice for each character, then generate a sample. voice tags help you quickly distinguish narrator, lead, villain, and supporting roles.</div>
              <div class="voice-stage-stats">
                <div class="voice-stage-stat">
                  <span class="voice-stage-stat-label">Assigned</span>
                  <strong>{{ charsvoiced }}/{{ chars.length }}</strong>
                </div>
                <div class="voice-stage-stat">
                  <span class="voice-stage-stat-label">Voice sample</span>
                  <strong>{{ voicesampleCount }}/{{ charsvoiced }}</strong>
                </div>
              </div>
              <div class="voice-library-meta">
                <span>voice Library</span>
                <span>{{ voiceProfiles.length }} entries</span>
              </div>
                  ...
                  <div class="voice-char">
                    ...
                    <div class="voice-name">
                      <div class="voice-name-row">
                        <div class="extract-name">{{ c.name }}</div>
                        <span class="tag" :class="(c.voice_style || c.voiceStyle) ? 'tag-success' : ''">{{ (c.voice_style || c.voiceStyle) ? 'Assigned' : 'Pending' }}</span>
                      </div>
                      <div class="extract-meta">{{ c.role || 'character' }}</div>
                    </div>
                  </div>
                </div>

                <div class="voice-card-copy">
                  <div class="voice-card-text">{{ c.description || c.personality || c.appearance || 'No character description. You can manually pick a voice based on the character role.' }}</div>
                </div>

                <div class="voice-select-block">
                  <span class="voice-block-label">Select voice</span>
                  <BaseSelect
        ...
        <div v-else-if="scriptStep === 4" class="step-editor">
          <div class="step-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">storyboard List</span>
              </div>
            </div>
            <div class="toolbar-right">
              <span v-if="sbs.length" class="char-count">{{ sbs.length }} shot · {{ totalDuration }}s</span>
              ...
              <template v-if="!sbs.length">
                <span class="locked-config">video model · {{ lockedVideoConfigLabel }}</span>
              </template>
              <button class="btn btn-sm" :disabled="rn" @click="dobreakdown">
                ...
                {{ sbs.length ? 'Re-break' : 'AI break storyboard' }}
              </button>
            </div>
          </div>
          ...
          <div v-if="sbs.length" class="split-layout">
            ...
            <div class="shot-list">
              <div class="shot-list-head">
                <div>
                  <div class="shot-list-title">shot Sequence</div>
                  <div class="shot-list-sub">Review content and asset state in shot order</div>
                </div>
              ...
              <div class="shot-list-body">
                ...
                >
                  <div class="shot-item-header">
                    ...
                    <span v-if="getstoryboardcharacterIds(sb).length" class="tag" style="font-size:10px">{{ getstoryboardcharacterIds(sb).length }} character</span>
                    <div class="shot-status">
                      <div v-if="sb.imageUrl || sb.composedimage || sb.firstFrameimage" class="shot-dot has-img" title="generated image"></div>
                      <div v-if="sb.videoUrl || sb.composedVideoUrl" class="shot-dot has-video" title="alreadygenerate Video"></div>
                      <div v-if="sb.dialogue" class="shot-dot has-dialogue" title="Has dialogue"></div>
                    </div>
                  </div>
                  <div class="shot-body">
                    <div class="shot-desc">{{ sb.description || sb.title || 'No description' }}</div>
                  </div>
                  <div class="shot-meta">
            ...
            <div class="detail-panel" v-if="selectedSb">
                <div class="detail-head">
                  <div class="detail-head-copy">
                    <span class="detail-head-title">shot #{{ sbs.indexOf(selectedSb) + 1 }}</span>
                  <span class="detail-head-sub">{{ selectedSb.title || `shot ${sbs.indexOf(selectedSb) + 1}` }} · {{ selectedSb.shot_type || selectedSb.shotType || 'No framing set' }}</span>
                  </div>
              ...
              <div class="detail-body">
                <div class="detail-hero">
                  <div class="detail-hero-copy">
                    <div class="detail-hero-label">shot Overview</div>
                    <div class="detail-hero-text">{{ selectedSb.description || selectedSb.title || 'This shot has no visual description yet. We recommend adding the core action and composition first.' }}</div>
                    <div class="detail-status-row">
                      <span class="tag">{{ getsceneName(selectedSb) }}</span>
                      <span class="tag">{{ selectedSb.angle || 'No angle' }}</span>
                      <span class="tag">{{ selectedSb.movement || 'No camera move' }}</span>
                      <span class="tag" :class="getFirstFrame(selectedSb) ? 'tag-success' : ''">First Frame {{ getFirstFrame(selectedSb) ? 'generated' : 'Pending' }}</span>
                      <span class="tag" :class="getLastFrame(selectedSb) ? 'tag-success' : ''">Last Frame {{ getLastFrame(selectedSb) ? 'generated' : 'Pending' }}</span>
                      <span class="tag" :class="hasVid(selectedSb) ? 'tag-success' : ''">Video {{ hasVid(selectedSb) ? 'generated' : 'Pending' }}</span>
                    </div>
                  </div>
                  <div class="detail-preview-grid">
                    <div class="detail-preview-card">
                      <div class="detail-preview-title">First Frame</div>
                      <div class="detail-preview-media">
                        <img
                          v-if="getFirstFrame(selectedSb)"
                          :src="'/' + getFirstFrame(selectedSb)"
                          class="previewable-image"
                          @click.stop="openimageViewer('/' + getFirstFrame(selectedSb), `shot #${sbs.indexOf(selectedSb) + 1} First Frame`)"
                        />
                        <div v-else class="detail-preview-empty">Pending</div>
                      </div>
                    </div>
                    <div class="detail-preview-card">
                      <div class="detail-preview-title">Last Frame</div>
                      <div class="detail-preview-media">
                        <img
                          v-if="getLastFrame(selectedSb)"
                          :src="'/' + getLastFrame(selectedSb)"
                          class="previewable-image"
                          @click.stop="openimageViewer('/' + getLastFrame(selectedSb), `shot #${sbs.indexOf(selectedSb) + 1} Last Frame`)"
                        />
                        <div v-else class="detail-preview-empty">Pending</div>
                      </div>
                    </div>
                ...
                <div class="detail-section">
                  <div class="detail-section-head">
                    <span class="detail-section-title">shot Structure</span>
                    <span class="detail-section-copy">Framing, angle, camera move, scene binding, and duration</span>
                  </div>
                  <div class="field-grid field-grid-4">
                    <label class="field">
                      <span class="field-label">Title</span>
                      <input :value="selectedSb.title || ''" class="input"
                        @blur="updateField(selectedSb, 'title', $event.target.value)" placeholder="e.g. Approaching snow" />
                    </label>
                    <label class="field">
                      ...
                      <input
                        ...
                        class="input"
                        placeholder="Pick or type a framing"
                    ...
                    <label class="field">
                      ...
                      <input
                        ...
                        class="input"
                        placeholder="Pick or type an angle"
                    ...
                    <label class="field">
                      <span class="field-label">Camera Move</span>
                      <input
                        list="shot-movement-list"
                        :value="selectedSb.movement || ''"
                        class="input"
                        placeholder="Pick or type a camera move"
                  ...
                  <div class="field-grid field-grid-4">
                    <label class="field">
                      <span class="field-label">Bound characters</span>
                      <div class="role-pills">
                        ...
                        <span v-if="!chars.length" class="dim" style="font-size:12px">No characters in this episode yet</span>
                      </div>
                    </label>
                    <label class="field">
                      <span class="field-label">Bound scene</span>
                      <select class="input" :value="selectedSb.scene_id || selectedSb.sceneId || ''"
                        ...
                        <option value="">No scene bound</option>
                        <option v-for="scene in scenes" :key="scene.id" :value="scene.id">
                          {{ scene.location }} · {{ scene.time || 'No time set' }}
                        </option>
                      </select>
                    </label>
                    <label class="field">
                      ...
                      <input :value="selectedSb.location || ''" class="input"
                        @blur="updateField(selectedSb, 'location', $event.target.value)" placeholder="scene location" />
                    </label>
                    <label class="field">
                      <span class="field-label">Time</span>
                      <input :value="selectedSb.time || ''" class="input"
                        @blur="updateField(selectedSb, 'time', $event.target.value)" placeholder="e.g. Late night / Early morning" />
                    </label>
                    <label class="field">
                ...
                <div class="detail-section">
                  <div class="detail-section-head">
                    <span class="detail-section-title">Visual Semantics</span>
                    <span class="detail-section-copy">action, result, atmosphere, and dialogue</span>
                  </div>
                  <div class="field-grid field-grid-2">
                    <label class="field">
                      <span class="field-label">action</span>
                      <textarea :value="selectedSb.action || ''" class="textarea" rows="3"
                        @blur="updateField(selectedSb, 'action', $event.target.value)" placeholder="Who is doing what, what is the expression and action detail" />
                    </label>
                    <label class="field">
                      <span class="field-label">Result</span>
                      <textarea :value="selectedSb.result || ''" class="textarea" rows="3"
                        @blur="updateField(selectedSb, 'result', $event.target.value)" placeholder="Visual state or image result at the end of the shot" />
                    </label>
                  </div>
                  <div class="field-grid field-grid-2">
                    <label class="field">
                      ...
                      <textarea :value="selectedSb.description || ''" class="textarea" rows="4"
                        @blur="updateField(selectedSb, 'description', $event.target.value)" placeholder="Describe the image content..." />
                    </label>
                    <label class="field">
                      <span class="field-label">Atmosphere</span>
                      <textarea :value="selectedSb.atmosphere || ''" class="textarea" rows="4"
                        @blur="updateField(selectedSb, 'atmosphere', $event.target.value)" placeholder="Lighting, color tone, empty atmosphere, environmental atmosphere" />
                    </label>
                  </div>
                  <label class="field">
                    <span class="field-label">Dialogue / Narrator</span>
                    <textarea :value="selectedSb.dialogue || ''" class="textarea" rows="3"
                      @blur="updateField(selectedSb, 'dialogue', $event.target.value)" placeholder="charactername：Dialogue content or Narrator：Content" />
                  </label>
                </div>
                <div class="detail-section">
                  <div class="detail-section-head">
                    <span class="detail-section-title">Generation Prompts</span>
                    <span class="detail-section-copy">Drives image, video, music, and sound-effect generation</span>
                  </div>
                  <label class="field">
                    ...
                    <textarea :value="selectedSb.image_prompt || selectedSb.imagePrompt || ''" class="textarea" rows="4"
                      @blur="updateField(selectedSb, 'image_prompt', $event.target.value)" placeholder="useatFirst Frame、Last Single-frame prompt for first frame, last frame, and shot image" />
                  </label>
                  <label class="field">
                  ...
                  <div class="field-grid field-grid-2">
                    <label class="field">
                      <span class="field-label">Music Prompt</span>
                      <textarea :value="selectedSb.bgm_prompt || selectedSb.bgmPrompt || ''" class="textarea" rows="3"
                        @blur="updateField(selectedSb, 'bgm_prompt', $event.target.value)" placeholder="e.g.Suppressed low-frequency strings，slow push-in" />
                    </label>
                    <label class="field">
                      <span class="field-label">Sound Effect Prompt</span>
                      <textarea :value="selectedSb.sound_effect || selectedSb.soundEffect || ''" class="textarea" rows="3"
                        @blur="updateField(selectedSb, 'sound_effect', $event.target.value)" placeholder="e.g.wind and snowvoice、stepping on snow、cloth rustlingvoice" />
                    </label>
                  </div>
                </div>
              </div>
          ...
          <div v-else-if="rn && rt === 'storyboard_breaker'" class="step-loading">
            <Loader2 :size="24" class="animate-spin" style="color:var(--accent)" />
            <div class="loading-text">isinbreak storyboardandgenerate Prompt...</div>
          </div>
          ...
          <div v-else class="step-empty">
            ...
            <div class="empty-title">break the script into a storyboard sequence</div>
            <div class="empty-desc">AI auto-analyzes the script, generates a shot list and video prompts</div>
            <div class="locked-config-banner">currentepisodevideo model：{{ lockedVideoConfigLabel }}</div>
      ...
      <div v-else-if="panel === 'production'" class="content-panel">
        ...
        <div v-if="!scriptContent || !sbs.length" class="step-empty" style="flex:1">
          ...
          <div class="empty-title">Not ready yet</div>
          <div class="empty-desc">{{ !scriptContent ? 'Please firstdonescriptcompilewrite' : 'Please finish storyboard breaking first' }}</div>
          <button class="btn btn-primary" @click="panel = 'script'">Go to script</button>
        </div>
        ...
        <template v-else>
          <div class="step-toolbar prod-toolbar">
            <div class="toolbar-left">
              <div class="step-indicator">
                ...
                <span class="step-name">production Workbench</span>
              </div>
            </div>
            <div class="prod-tabs">
              <button
          ...
          <div v-if="prodTab === 'chars'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ visualChars.length }} characters need generated portraits</span>
              <span class="tag">{{ lockedimageConfigLabel }}</span>
              <span v-if="chars.length > visualChars.length" class="tag">Narrator keeps voice only</span>
              ...
              <div v-for="c in visualChars" :key="c.id" class="card asset-card">
                <div class="asset-cover">
                  <img
                    ...
                    class="previewable-image"
                    @click.stop="openimageViewer('/' + (c.image_url || c.imageUrl), `${c.name} character portrait`)"
                  />
                  ...
                  <span class="asset-cover-badge" :class="(c.image_url || c.imageUrl) ? 'is-ready' : (isPendingCharimage(c.id) ? 'is-pending' : '')">{{ (c.image_url || c.imageUrl) ? 'generated' : (isPendingCharimage(c.id) ? 'Generating' : 'Pending') }}</span>
                </div>
                <div class="asset-body">
                  <div class="asset-name">{{ c.name }}</div>
                  <div class="asset-meta dim">{{ c.role || 'character' }}</div>
                </div>
                <div class="asset-foot">
                  <span :class="['dot', (c.image_url || c.imageUrl) && 'ok', isPendingCharimage(c.id) && 'pending']" />
                  <span class="dim" style="font-size:10px">{{ (c.image_url || c.imageUrl) ? 'generated' : (isPendingCharimage(c.id) ? 'Generating' : 'Pending') }}</span>
                  <button class="btn btn-sm ml-auto" :disabled="isPendingCharimage(c.id)" @click="genCharImg(c.id)">{{ isPendingCharimage(c.id) ? 'Generating' : 'generate' }}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="prodTab === 'scenes'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ scenes.length }} scenes</span>
              <span class="tag">{{ lockedimageConfigLabel }}</span>
              <div class="ml-auto flex gap-1">
              ...
              <div v-for="s in scenes" :key="s.id" class="card asset-card">
                <div class="asset-cover wide">
                  <img
                    ...
                    class="previewable-image"
                    @click.stop="openimageViewer('/' + (s.image_url || s.imageUrl), `${s.location} sceneimage`)"
                  />
                  <div v-else class="asset-cover-empty">
                  ...
                  <span class="asset-cover-badge" :class="(s.image_url || s.imageUrl) ? 'is-ready' : (isPendingsceneimage(s.id) ? 'is-pending' : '')">{{ (s.image_url || s.imageUrl) ? 'generated' : (isPendingsceneimage(s.id) ? 'Generating' : 'Pending') }}</span>
                </div>
                <div class="asset-body">
                ...
                <div class="asset-foot">
                  ...
                  <span class="dim" style="font-size:10px">{{ (s.image_url || s.imageUrl) ? 'generated' : (isPendingsceneimage(s.id) ? 'Generating' : 'Pending') }}</span>
                  <button class="btn btn-sm ml-auto" :disabled="isPendingsceneimage(s.id)" @click="gensceneImg(s.id)">{{ isPendingsceneimage(s.id) ? 'Generating' : 'generate' }}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="prodTab === 'dubbing'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ ttsEligibleCount }} entries can synthesize voice</span>
              <span class="tag mono">{{ ttsgeneratedCount }}/{{ ttsEligibleCount }} generated</span>
            ...
            <div v-if="!ttsEligibleCount" class="step-empty" style="min-height:260px">
              ...
              <div class="empty-title">No dialogue to synthesize currently</div>
              <div class="empty-desc">Fill in "Character name: line" or "Narrator: copy" in the storyboard first; voice shots to synthesize will appear here.</div>
            </div>
            ...
            <div v-else class="dub-grid">
                <div v-for="(sb, i) in sbs.filter(hasDialogue)" :key="sb.id" class="card dub-card">
                  <div class="dub-head">
                    ...
                    <div class="dub-desc">{{ getDialogueText(sb) || 'No text' }}</div>
                    </div>
                    <span class="tag" :class="hasTTS(sb) ? 'tag-success' : ''">{{ hasTTS(sb) ? 'generated' : 'Pending' }}</span>
                  </div>
                <div class="dub-meta">
                  <span class="dim">{{ sb.shot_type || sb.shotType || 'not setframing' }}</span>
                  <span class="dim">{{ sb.duration || 10 }}s</span>
                  <span class="dim">{{ sb.location || 'not setclick' }}</span>
                </div>
                <div class="dub-foot">
                  <audio v-if="hasTTS(sb)" :src="'/' + getTTSUrl(sb)" controls preload="none" class="dub-audio" />
                  <div v-else class="dim" style="font-size:12px">No voice file generated yet</div>
                  <button class="btn btn-sm ml-auto" @click="genshotTTS(sb)">generate voice</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="prodTab === 'shots'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ sbs.length }} shots</span>
              <span class="tag mono">{{ shotImgCount }}/{{ sbs.length }} shots with frames</span>
              <span class="tag">{{ lockedimageConfigLabel }}</span>
              <div class="ml-auto flex gap-1">
                <BaseSelect v-model="frameMode" :options="frameModeOptions" placeholder="frame mode" searchable style="width:100px" />
            ...
            <div v-if="gridHistory.length" class="grid-history-panel">
              <div v-if="gridimagePath" class="latest-grid-strip">
                <button class="latest-grid-strip-thumb" @click="openimageViewer('/' + gridimagePath, 'Current grid image')">
                ...
                <div class="latest-grid-strip-copy">
                  ...
                  <div class="latest-grid-strip-title">Current grid image</div>
                  <div class="latest-grid-strip-meta">
                    <span v-if="gridRecoveredAt">{{ gridRecoveredAt }}</span>
                    <span>Can continue splitting and assigning</span>
                  </div>
                </div>
                <div class="latest-grid-strip-actions">
                  <button class="btn btn-sm" @click="reopengridPreview">Preview</button>
                  <button class="btn btn-primary btn-sm" @click="continuegridSplit">Continue splitting</button>
                </div>
              </div>
              <div class="grid-history-head">
                ...
                <button class="btn btn-sm" @click="showAllgridHistory = !showAllgridHistory">
                  ...
                  {{ showAllgridHistory ? 'receiverisegrid image History' : `Expand All (${gridHistory.length})` }}
                </button>
              </div>
                ...
                <div v-for="(sb, i) in sbs" :key="sb.id"
                  ...
                  <div class="frame-info">
                    ...
                    <div class="frame-meta">
                      ...
                      <span class="dim" style="font-size:11px">First Frame</span>
                      <span v-if="frameMode === 'first_last'" style="display:flex;align-items:center;gap:4px">
                        ...
                        <span class="dim" style="font-size:11px">Last Frame</span>
                      </span>
                    </div>
                  </div>
                  <!-- Thumbnails -->
                    ...
                    <div class="frame-thumb-wrap">
                      <div class="frame-thumb" @click.stop="!isPendingshotFrame(sb.id, 'first_frame') && genshotFrame(sb, 'first_frame')">
                        <img
                          ...
                          class="previewable-image"
                          @click.stop="openimageViewer('/' + getFirstFrame(sb), `shot #${String(i + 1).padStart(2, '0')} First Frame`)"
                        />
                      ...
                      <span class="frame-thumb-label">{{ isPendingshotFrame(sb.id, 'first_frame') ? 'First FrameGenerating' : 'First Frame' }}</span>
                    </div>
                    <div v-if="frameMode === 'first_last'" class="frame-thumb-wrap">
                      <div class="frame-thumb" @click.stop="!isPendingshotFrame(sb.id, 'last_frame') && genshotFrame(sb, 'last_frame')">
                        <img
                          ...
                          :src="'/' + getLastFrame(sb)"
                          class="previewable-image"
                          @click.stop="openimageViewer('/' + getLastFrame(sb), `shot #${String(i + 1).padStart(2, '0')} Last Frame`)"
                        />
                        <div v-else class="frame-thumb-empty">
                      ...
                      <span class="frame-thumb-label">{{ isPendingshotFrame(sb.id, 'last_frame') ? 'Generating last frame' : 'Last Frame' }}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-if="gridDialog" class="overlay" @click.self="gridDialog = false">
              <div class="card grid-tool">
                <div class="grid-tool-head">
                  <span style="font-size:15px;font-weight:600;font-family:var(--font-display)">grid image Tool</span>
                ...
                <div v-if="gridStep === 0" class="grid-tool-body">
                  ...
                  <div class="grid-config">
                    <label class="field" style="flex:0 0 auto" v-if="gridMode !== 'multi_ref'">
                      ...
                      <BaseSelect v-model="gridLayout" :options="gridLayoutOptions" placeholder="grid" style="width:90px" />
                    </label>
                    <div class="field" style="flex:1">
                      <span class="field-label">
                        {{ gridMode === 'multi_ref' ? 'Selecttargetshot' : 'Select shot' }}
                        <span class="dim" v-if="gridMode !== 'multi_ref'">(alreadyselect {{ gridSelected.length }})</span>
                      </span>
                    </div>
                    <div style="align-self:flex-end" v-if="gridMode !== 'multi_ref'">
                      <button class="btn btn-sm" @click="gridSelectAll">{{ gridSelected.length === sbs.length ? 'CancelSelect All' : 'Select All' }}</button>
                    </div>
                  </div>
                  ...
                  <div class="grid-tool-foot">
                    <span v-if="gridCanStart" class="tag mono">{{ gridAutoLayout.rows }}x{{ gridAutoLayout.cols }} = {{ gridAutoLayout.rows * gridAutoLayout.cols }}cell</span>
                    ...
                    <button class="btn btn-primary ml-auto" :disabled="!gridCanStart || gridPromptLoading" @click="generategridPrompt">
                      ...
                      {{ gridPromptLoading ? 'Generating' : 'generate Prompt' }}
                    </button>
                  </div>
                </div>
                ...
                <div v-else-if="gridStep === 1" class="grid-tool-body">
                  <div class="grid-prompt-summary">
                    <div class="grid-prompt-label">
                      ...
                      grid imageprompt
                      <span v-if="gridPromptSource" class="tag ml-8">{{ gridPromptSource === 'agent' ? 'AIgenerate' : 'template fallback' }}</span>
                    </div>
                    <div class="grid-prompt-text">{{ gridPromptText || '（etc.Pending）' }}</div>
                  </div>

                  <div class="grid-blank-preview" :style="gridBlankStyle">
                    <div v-for="(cell, i) in gridCellPrompts" :key="i" class="grid-blank-cell">
                      <div class="grid-blank-cell-index">#{{ cell.shot_number }} {{ {first_frame:'First Frame',last_frame:'Last Frame',reference:'Reference'}[cell.frame_type] || '' }}</div>
                    ...
                    <div v-for="i in Math.max(0, (gridAutoLayout.rows * gridAutoLayout.cols) - gridCellPrompts.length)" :key="'empty-'+i" class="grid-blank-cell empty">
                      <div class="grid-blank-cell-index">Empty</div>
                      <div class="grid-blank-cell-desc">—</div>
                    </div>
                  </div>

                  <div class="grid-tool-foot">
                    <button class="btn" @click="gridStep = 0">Previous</button>
                ...
                <div v-else-if="gridStep === 2" class="grid-tool-body" style="align-items:center;justify-content:center;min-height:300px">
                  ...
                  <div class="loading-text" style="margin-top:12px">Generating grid image...</div>
                ...
                <div v-else-if="gridStep === 3" class="grid-tool-body grid-tool-body-preview">
                  <div class="grid-preview-layout">
                    <div class="grid-preview-pane">
                      <div class="grid-preview-wrap">
                        <div class="grid-preview-stage">
                          <img
                            ...
                            @click.stop="openimageViewer('/' + gridimagePath, 'Grid image preview')"
                          />
                      ...
                      <div class="grid-adjust-summary">
                        <span class="tag mono">{{ gridActualLayout.rows }}x{{ gridActualLayout.cols }} = {{ gridActualLayout.rows * gridActualLayout.cols }}cell</span>
                        <span class="dim" style="font-size:12px">{{ gridAssignedCount }}/{{ gridAssignments.length }} cells assigned</span>
                        <span class="tag" v-if="gridAssignedCount < gridAssignments.length">Unassigned cells are ignored and not written back to the storyboard</span>
                      </div>
                    </div>
                    <div class="grid-assignment-pane">
                      <div class="grid-assign-head">
                        <div class="grid-assign-title">Cell Assignment</div>
                        <div class="grid-assign-subtitle">After splitting, decide which storyboard each cell binds to</div>
                      </div>
                      <div v-if="gridAssignmentTotalPages > 1" class="grid-assign-pagination">
                        <button class="btn btn-sm" :disabled="gridAssignmentPage === 0" @click="gridAssignmentPage--">Previous</button>
                        <span class="dim">No. {{ gridAssignmentPage + 1 }}/{{ gridAssignmentTotalPages }} page</span>
                        <span class="dim">{{ gridAssignmentPageStart + 1 }}-{{ gridAssignmentPageEnd }} / {{ gridAssignments.length }}</span>
                        <button class="btn btn-sm ml-auto" :disabled="gridAssignmentPage >= gridAssignmentTotalPages - 1" @click="gridAssignmentPage++">Next</button>
                      </div>
                      ...
                      <div class="grid-assign-info">
                        <div v-for="item in pagedgridAssignments" :key="item.index" :class="['grid-assign-row', activegridCell === item.index && 'active']">
                          <span class="grid-assign-index">Cell {{ item.index + 1 }}</span>
                          <BaseSelect
                  ...
                  <div class="grid-tool-foot">
                    <button class="btn" @click="gridStep = 1">Back</button>
                ...
                <div v-else-if="gridStep === 4" class="grid-tool-body" style="align-items:center;justify-content:center;min-height:200px">
                  ...
                  <div style="font-size:17px;font-weight:700;font-family:var(--font-display);margin-top:8px">Assignment Complete</div>
                  <div class="dim" style="font-size:13px;margin-top:4px">{{ gridAssignedCount }} cells assigned</div>
                  <button class="btn btn-primary" style="margin-top:16px" @click="gridDialog = false; refresh()">Close</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="prodTab === 'videos'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ sbs.length }} shots</span>
              <span class="tag mono">{{ shotVidCount }}/{{ sbs.length }} generated</span>
              <div class="ml-auto flex gap-1">
              ...
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                <div class="prod-cover">
                  ...
                  <img
                    ...
                    class="previewable-image"
                    @click.stop="openimageViewer('/' + getstoryboardCover(sb), `shot #${String(i + 1).padStart(2, '0')} Reference image`)"
                  />
                  ...
                  <span v-if="hascomposed(sb)" class="prod-overlay-badge">alreadycompose</span>
                </div>
                <div class="prod-info">
                  <div class="prod-desc truncate">{{ sb.description || sb.title || '—' }}</div>
                  <div class="prod-meta-line">{{ sb.shot_type || sb.shotType || 'No framing' }} · {{ sb.duration || 10 }}s</div>
                  <div class="prod-dots">
                    <span :class="['dot', hasImg(sb) && 'ok']" /><span style="font-size:10px">image</span>
                    <span :class="['dot', hasVid(sb) && 'ok', isPendingVideo(sb.id) && 'pending']" /><span style="font-size:10px">{{ isPendingVideo(sb.id) ? 'VideoGenerating' : 'Video' }}</span>
                  </div>
                ...
                <div class="prod-actions">
                  <button class="btn btn-sm" :disabled="isPendingVideo(sb.id)" @click="genVid(sb)">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
                    {{ isPendingVideo(sb.id) ? 'Generating' : 'generate Video' }}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div v-else-if="prodTab === 'compose'" class="prod-content">
            <div class="prod-section-bar">
              <span class="dim" style="font-size:12px">{{ sbs.length }} shots</span>
              <span class="tag mono">{{ composedCount }}/{{ sbs.length }} alreadycompose</span>
              <div class="ml-auto flex gap-1">
            ...
            <div class="prod-grid">
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                ...
                <div class="prod-info">
                  ...
                  <div class="prod-meta-line">{{ sb.shot_type || sb.shotType || 'No framing' }} · {{ sb.duration || 10 }}s</div>
                  <div class="prod-dots">
                    <span :class="['dot', hasVid(sb) && 'ok']" /><span style="font-size:10px">Video</span>
                    <span :class="['dot', hasTTS(sb) && 'ok']" /><span style="font-size:10px">voice</span>
                    <span :class="['dot', hascomposed(sb) && 'ok', isPendingcompose(sb.id) && 'pending']" /><span style="font-size:10px">{{ isPendingcompose(sb.id) ? 'composing' : 'compose' }}</span>
                  </div>
                ...
                <div class="prod-actions">
                  <button class="btn btn-sm" :disabled="!hasVid(sb) || isPendingcompose(sb.id)" @click="docompose(sb)">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                    {{ isPendingcompose(sb.id) ? 'composing' : (hascomposed(sb) ? 'Re-compose' : 'Start Composing') }}
                  </button>
                </div>
              </div>
            </div>
      ...
      <div v-else class="content-panel">
        <div v-if="!sbs.length" class="step-empty" style="flex:1">
          ...
          <div class="empty-title">Not ready yet</div>
          <div class="empty-desc">Please complete storyboard breaking and production flow first</div>
          <button class="btn btn-primary" @click="panel = 'script'">Go to script</button>
        </div>
        <div v-else class="export-split">
          <div class="export-main">
            <template v-if="mergeUrl">
              ...
              <div class="export-bar">
                <span class="tag tag-success">merge Complete</span>
                <span class="dim" style="font-size:12px">{{ sbs.length }} shot · {{ totalDuration }}s</span>
            ...
            <template v-else>
              <div class="step-empty">
                ...
                <div class="empty-title">merge full episode video</div>
                <div class="empty-desc">merge {{ composedCount }} composed shots into a full video</div>
          ...
          <div class="export-list">
            <div class="export-list-head">shot Overview</div>
      ...
      <div v-if="showBottomBubble" class="step-bubble">
        ...
        >
          ...
          {{ prodTabDefs[Math.max(0, prodTabIdx - 1)]?.label || 'Previous' }}
        </button>
        ...
        <button
          ...
          @click="goNextProd"
        >
          {{ prodTabIdx < prodTabDefs.length - 1 ? (prodTabDefs[prodTabIdx + 1]?.label || 'Next') : 'Enter export' }}
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/>
          </svg>
        </button>
      ...
      <div v-if="imageViewer.open && imageViewer.src" class="overlay image-viewer-overlay" @click.self="closeimageViewer">
        <div class="card image-viewer-dialog">
          <div class="image-viewer-head">
            <div class="image-viewer-title">{{ imageViewer.title || 'Image preview' }}</div>
            <button class="btn btn-ghost btn-icon" @click="closeimageViewer">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div class="image-viewer-body">
            <img :src="imageViewer.src" :alt="imageViewer.title || 'Image preview'" class="image-viewer-img" />
          </div>
        </div>
      </div>
    </main>
...
const fallbackvoiceProfiles = [
  { id: 'alloy', label: 'Alloy', gender: 'Neutral', traits: 'Balanced, natural, reserved', suitable: 'Narration, narrator, supporting characters who need stable output' },
  { id: 'echo', label: 'Echo', gender: 'Male', traits: 'Deep、Steady、Calm', suitable: 'Mature male, father figure, narrator, oppressive characters' },
  { id: 'fable', label: 'Fable', gender: 'Male', traits: 'Warm、narrative feel、expressive', suitable: 'Male Lead、growth-oriented character、Narrator' },
  { id: 'onyx', label: 'Onyx', gender: 'Male', traits: 'Deep, powerful, authoritative', suitable: 'Villains, assertive characters, controller-type characters' },
  { id: 'nova', label: 'Nova', gender: 'Female', traits: 'Gentle, sweet, approachable', suitable: 'Female lead, mother, gentle and supporting characters' },
  { id: 'shimmer', label: 'Shimmer', gender: 'Female', traits: 'Bright, lively, youthful', suitable: 'Young women, lively characters, unconventional supporting roles' },
]
...
const frameModeOptions = [{ label: 'First frame only', value: 'first' }, { label: 'FirstLast Frame', value: 'first_last' }]
...
function configLabel(config) {
  if (!config) return 'notConfiguration'
  let modelName = ''
...
function isNarratorcharacter(char) {
  ...
  return text.includes('Narrator') || text.includes('narrator') || text.includes('imageoutsidesound')
}
...
const gridSummary = computed(() => {
  if (gridMode.value === 'multi_ref') {
    ...
    return gridSingleTarget.value ? `${gridLayoutShape.value.rows}x${gridLayoutShape.value.cols} Reference image → shot #${idx}` : 'Please select a shot'
  }
  if (!gridSelected.value.length) return 'pleaseSelect shot'
  const count = gridSelected.value.length
  if (gridMode.value === 'first_last') {
    const { rows, cols } = gridLayoutShape.value
    return `${count} shots → ${rows}x${cols} grid（PressFirstLast Framewindcellgenerate，split first then assign manually）`
  }
  const { rows, cols } = gridLayoutShape.value
  const cells = rows * cols
  return `${count} shots → ${rows}x${cols} grid（firstgenerategrid image，split first then assign manually）`
})
...
const gridAssignmentshotOptions = computed(() => [
  ...
  ...gridAssignableshotIds.value.map((id) => {
    ...
    return {
      label: `#${String(index).padStart(2, '0')} ${sb?.title || sb?.description || 'shot'}`,
      value: id,
    }
  }),
])
...
function gridCellLabel(a) {
  if (!a?.storyboard_id) return 'Unassigned'
  const idx = sbs.value.findIndex(s => s.id === a.storyboard_id) + 1
  const suffix = { first_frame: 'First', last_frame: 'Last', reference: 'Reference' }[a.frame_type] || ''
...
function gridCellTitle(id) {
  ...
  const sb = sbs.value.find(s => s.id === id)
  return `#${String(idx).padStart(2, '0')} ${sb?.title || sb?.description || 'shot'}`
}
...
const stepLabels = ['Original Content', 'AI Rewrite', 'Extract', 'voice', 'storyboard']
const prevStepLabel = computed(() => scriptStep.value > 0 ? stepLabels[scriptStep.value - 1] : '')
const nextStepLabel = computed(() => {
  if (scriptStep.value === 4) return 'enterinproduction'
...
async function generategridPrompt() {
  if (!gridCanStart.value) {
    toast.warning('Please firstSelect shot')
    return
  }
  gridPromptLoading.value = true
  gridPromptStatus.value = 'isintoneuse AI generategridprompt...'
  gridPromptText.value = ''
  gridCellPrompts.value = []
  gridPromptSource.value = ''
  try {
    ...
    if (gridPromptText.value) {
      ...
      gridPromptStatus.value = gridPromptSource.value === 'agent' ? 'AI promptgenerated' : 'Template prompt used'
      gridStep.value = 1
    } else {
      gridPromptStatus.value = ''
      toast.error('promptgeneratefailed')
    }
  } catch (e) {
    gridPromptStatus.value = ''
    toast.error(e?.message || 'generate Promptfailed')
  } finally {
...
async function startgridGen() {
  ...
  if (!gridAssignmentsState.value.length) resetgridAssignments()
  gridStep.value = 2
  gridStatusText.value = 'Submitting generation request...'
  try {
    ...
    gridStatusText.value = 'Waiting for image generation...'
    pollgridStatus()
  } catch (e) {
...
async function pollgridStatus() {
  for (let i = 0; i < 120; i++) {
    ...
    try {
      ...
      gridStatusText.value = `Status: ${res.status}`
      ...
      if (res.status === 'failed') {
        toast.error(res.error_msg || 'generatefailed')
        gridStep.value = 0
        return
      }
    } catch {}
  }
  toast.error('Generation timed out'); gridStep.value = 0
}
...
const mainStageDefs = [
  { id: 'script', label: 'script', desc: 'ContentRewriteand organize', icon: FileText },
  { id: 'assets', label: 'Assets', desc: 'Characters, scenes, and voices', icon: FolderKanban },
  { id: 'storyboard', label: 'storyboard', desc: 'shotproductionandcompose', icon: Clapperboard },
  { id: 'export', label: 'export', desc: 'merge and export the final video', icon: Download },
]
...
const sidebarSections = computed(() => ([
  {
    ...
    label: 'script',
    items: [
      { key: 'script:raw', label: 'Original Content', desc: '', icon: FileText, done: !!rawContent.value },
      { key: 'script:rewrite', label: 'AI Rewrite', desc: '', icon: FileText, done: !!scriptContent.value },
      { key: 'script:extract', label: 'Extract', desc: '', icon: Users, done: !!chars.value.length },
      { key: 'script:voice', label: 'voice', desc: '', icon: Mic2, done: !!chars.value.length && charsvoiced.value === chars.value.length },
      { key: 'script:storyboard', label: 'storyboard', desc: '', icon: Clapperboard, done: !!sbs.value.length },
    ],
  },
  {
    id: 'production',
  ...
  },
  {
    ...
    label: 'export',
    items: [
      { key: 'export:merge', label: 'mergeexport', desc: '', icon: Download, done: !!mergeUrl.value },
    ],
  },
]))
...
const activeSubSteps = computed(() => {
  if (activeMainStage.value === 'script') {
    return [
      { key: 'script:raw', label: 'Original Content', done: !!rawContent.value },
      { key: 'script:rewrite', label: 'AI Rewrite', done: !!scriptContent.value },
    ]
  }
  if (activeMainStage.value === 'assets') {
    return [
      { key: 'script:extract', label: 'Extractcharacterscene', done: !!chars.value.length },
      { key: 'script:voice', label: 'Assign voices', done: !!chars.value.length && charsvoiced.value === chars.value.length },
      { key: 'prod:chars', label: 'Character Portraits', done: !visualCharTotal.value || charImgCount.value === visualCharTotal.value },
      { key: 'prod:scenes', label: 'sceneimage', done: !scenes.value.length || sceneImgCount.value === scenes.value.length },
    ]
  }
  if (activeMainStage.value === 'storyboard') {
    return [
      { key: 'script:storyboard', label: 'Storyboard Breaking', done: !!sbs.value.length },
      { key: 'prod:dubbing', label: 'voicegenerate', done: !ttsEligibleCount.value || ttsgeneratedCount.value === ttsEligibleCount.value },
      { key: 'prod:shots', label: 'shotimage', done: !!sbs.value.length && shotImgCount.value === sbs.value.length },
      { key: 'prod:videos', label: 'Videogenerate', done: !!sbs.value.length && shotVidCount.value === sbs.value.length },
      { key: 'prod:compose', label: 'Videocompose', done: !!sbs.value.length && composedCount.value === sbs.value.length },
    ]
  }
  return [
    { key: 'export:merge', label: 'mergeexport', done: !!mergeUrl.value },
  ]
})
...
const bubbleSteps = computed(() => {
  if (panel.value === 'script') {
    return [
      { key: 'script:raw', label: 'Original Content', done: !!rawContent.value },
      { key: 'script:rewrite', label: 'AI Rewrite', done: !!scriptContent.value },
      { key: 'script:extract', label: 'Extract', done: !!chars.value.length },
      { key: 'script:voice', label: 'voice', done: !!chars.value.length && charsvoiced.value === chars.value.length },
      { key: 'script:storyboard', label: 'storyboard', done: !!sbs.value.length },
    ]
  }
  if (panel.value === 'production') {
...
const currentStageLabel = computed(() => {
  if (panel.value === 'script') return `script section · ${stepLabels[scriptStep.value]}`
  if (panel.value === 'production') return `production section · ${prodTabDefs.value[prodTabIdx.value]?.label || 'production'}`
  return mergeUrl.value ? 'export Stage · final video generated' : 'export Stage · Waiting to merge'
})

const currentMainStageLabel = computed(() => {
  const current = mainStageDefs.find(stage => stage.id === activeMainStage.value)
  return current?.label || 'Workbench'
})
...
const shotTypes = [
  'bigExtreme Wide', 'Extreme Wide', 'Wide', 'Medium', 'Medium-Close', 'Close-Up', 'Extreme Close-Up', 'bigExtreme Close-Up',
  'two-personshot', 'three-personshot', 'ensemble', 'back view', 'Side View', 'isface', 'Top-Down', 'Low Angle',
  'shoulder', 'POV', 'aerial', 'movementshot',
]
const shotAngles = ['Eye Level', 'Low Angle', 'Top-Down', 'Side shot', 'back shot', 'diagonal side', 'POV', 'shoulder']
const shotMovements = ['Static', 'Push-In', 'Pull-Out', 'Pan', 'Tracking', 'Follow', 'Crane', 'Handheld', 'Orbit']
...
function getsceneName(sb) {
  ...
  if (!sceneId) return 'No scene bound'
  const scene = scenes.value.find(s => s.id === sceneId)
  return scene ? `${scene.location} · ${scene.time || 'not setTime'}` : `scene #${sceneId}`
}

async function deleteshot(sb) {
  if (!confirm('ConfirmDeletethisshot？')) return
...
const scriptSteps = computed(() => {
  ...
  return [
    { label: 'Original Content', state: rawContent.value ? 'done' : 'active', spinning: false },
    { label: 'AI Rewrite', state: hasscript ? 'done' : (rawContent.value ? 'active' : ''), spinning: rt.value === 'script_rewriter' },
    { label: 'Extract', state: hasChars ? 'done' : (hasscript ? 'active' : ''), spinning: rt.value === 'extractor' },
    { label: 'voice', state: hasvoice ? 'done' : (hasChars ? 'active' : ''), spinning: rt.value === 'voice_assigner' },
    { label: 'storyboard', state: hasSbs ? 'done' : (hasvoice ? 'active' : ''), spinning: rt.value === 'storyboard_breaker' },
  ]
})
...
function doRewrite() { saveRaw(); runAgent('script_rewriter', 'please read the script and rewrite it into a formatted script, then save it', dramaId, epId.value, refresh) }
function skipRewrite() {
  const raw = (localRaw.value || rawContent.value || '').trim()
  if (!raw) {
    toast.warning('Please fill in the original content first')
    return
  }
  localscript.value = raw
  saveScr()
  toast.success('Skipped AI rewrite; will use the original content directly')
  scriptStep.value = 2
}
function doExtract() { saveScr(); runAgent('extractor', 'Please extract all character and scene information from the script in progress. After extraction, automatically count and dedup against the project\'s existing data.', dramaId, epId.value, refresh) }
function dovoice() { runAgent('voice_assigner', 'pleaseissohascharacterassign suitablevoice', dramaId, epId.value, refresh) }
async function batchGensamples() {
  ...
  if (failCount) toast.error(`${failCount}  sample filesgeneratefailed`)
  await refresh()
}
...
const IGNORE_TTS_SPEAKERS = /^(environmentsound|environmentvoice|soundeffect|effectsound|sfx|sound ?effect|bgm|backgroundsound|backgroundsoundmusic|ambient)$/i
const IGNORE_TTS_TEXT = /^(none|no dialogue|noneDialogue|noneNarrator|neednovoice|neednoDialogue|none|null|n\/a|na|environmentsound|environmentvoice|soundeffect|effectsound|pureSFX|pureenvironmentsound|ambient only|ambient only|backgroundsound|backgroundsoundmusic|bgm|sfx|ambient)$/i
...
async function genshotTTS(sb) {
  try {
    ...
    toast.success(`shot #${sb.storyboard_number || sb.storyboardNumber || sb.id} voicegenerated`)
    await refresh()
...
async function batchshotTTS() {
  ...
  if (!pending.length) {
    toast.info(ttsEligibleCount.value ? 'sohasshotvoicegenerated' : 'No dialogue or narration to generate currently')
    return
  }
  const results = await Promise.allSettled(pending.map(sb => storyboardAPI.generateTTS(sb.id)))
  const okCount = results.filter(r => r.status === 'fulfilled').length
  const failCount = results.length - okCount
  if (okCount) toast.success(`generated ${okCount} entriesshotvoice`)
  if (failCount) toast.error(`${failCount} entriesshotvoicegeneratefailed`)
  await refresh()
}
...
function buildshotimagePrompt(sb, frameType) {
  ...
  const frameHint = frameType === 'first_frame'
    ? 'generate the start/end key frame of this shot, highlighting the relationship between close-up and action opening moment'
    : 'generate the ending key frame of this shot，highlight action ending、emotional landing or result state'

  return [
    title ? `Shot title: ${title}` : '',
    description ? `image description：${description}` : '',
    shotType ? `framing：${shotType}` : '',
    angle ? `Camera position: ${angle}` : '',
    movement ? `camera move：${movement}` : '',
    charactersText ? `character：${charactersText}` : '',
...
async function genshotFrame(sb, frameType) {
  ...
  try {
    ...
    toast.success(frameType === 'first_frame' ? 'First FrameGenerating' : 'Generating last frame')
    await refresh()
    watchAsyncResult(() => {
...
async function pollVideoGeneration(generationId, storyboardId) {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    try {
      ...
      if (res?.status === 'completed') {
        ...
        toast.success('Videogeneratedone')
        return
      }
      if (res?.status === 'failed') {
        pendingVideoIds.value = pendingVideoIds.value.filter(item => item !== storyboardId)
        failedVideoMessages.value = {
          ...failedVideoMessages.value,
          [storyboardId]: res?.error_msg || res?.errorMsg || 'Video generation failed',
        }
  ...
  failedVideoMessages.value = {
    ...failedVideoMessages.value,
    [storyboardId]: 'Video generation timed out',
  }
...
async function batchcompose() {
  ...
  toast.success('batch composealreadyopenbegin')
  pollcomposeStatus()
}
async function domerge() {
  await mergeAPI.merge(epId.value); toast.success('Merging...')
  const poll = setInterval(async () => {
    try { mergeData.value = await mergeAPI.status(epId.value) } catch {}
    if (mergeData.value?.status === 'completed' || mergeData.value?.status === 'failed') {
      clearInterval(poll)
      mergeData.value.status === 'completed' ? toast.success('merge Complete') : toast.error('merge failed')
    }
  }, 3000)
}
    ...
    try {
      ...
      if (failedItems.length) {
        ...
        failedItems.forEach((item) => {
          next[item.id] = item.error_msg || item.errorMsg || 'Videocomposefailed'
        })
        failedcomposeMessages.value = next
      }

      if (!processingIds.length) {
        if (failedItems.length) toast.error(`has ${failedItems.length} shotscomposefailed`)
        else toast.success('batch composedone')
        return
      }
    } catch {}
  }
...
function mapvoiceProfile(v) {
  ...
  return {
    ...
    suitable: desc.length > 2 ? desc.slice(2).join('、') : `${v.language || 'passuse'}character`,
  }
}
